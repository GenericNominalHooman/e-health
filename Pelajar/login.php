...
if(isset($_POST['submit'])){
    ...
    $katalaluan = mysqli_real_escape_string($conn, $_POST['katalaluanpelajar']);
 
    $select = mysqli_query($conn, "SELECT * FROM `loginpelajar` WHERE namapelajar  = '$namapelajar' AND katalaluanpelajar = '$katalaluan'") or die('query failed');
    if(mysqli_num_rows($select) > 0){
       $row = mysqli_fetch_assoc($select);
       $_SESSION['namapelajar'] = $namapelajar;
             header("Location: utama.php"); // Akan Tukar Nama dashboard iaitu site profile
           }else{
