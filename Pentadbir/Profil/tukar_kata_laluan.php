...
<?php
    ...
    if($_SESSION["Auth"]){
        if(isset($_POST["submit_tukar_kata_laluan"])){
            ...
            $kataLaluanLamaDB = $loginModel->getAllPentadbirWhere("id", $_SESSION["Auth"]["id"])[0]["katalaluanpentadbir"];
            ...
            // Update login model
            if($validInput){
                $loginModel->updateUser("loginpentadbir", "id", $_SESSION["Auth"]["id"], [
                    "katalaluanpentadbir" => password_hash($kataLaluanBaharu, PASSWORD_BCRYPT),
                ]);
