...
if (isset($_POST['submit'])) {
  ...
  if(!$verificationObj->isNameExist($_POST["namapentadbir"])){
    //Registering namapentadbir
    if ($authObj->registerPentadbir($_POST["namapentadbir"], $_POST["katalaluan"], "")) {
      // pentadbir successfully registered
      // Login pentadbir
      $authObj->authPentadbir($_POST["namapentadbir"], $_POST["katalaluan"]);
      Redirect::redirect(PENTADBIR_URL."/Profil/profil.php");
    } else {
      // pentadbir wasn't successfully registered
      Redirect::redirectWithMsg(PENTADBIR_URL . "/register.php", "Pendaftaran masuk gagal, tolong maklumkan pentadbir sistem berkaitan isu ini.");
    } 
  } else {
    // pentadbir wasn't successfully registered
    Redirect::redirectWithMsg(PENTADBIR_URL . "/register.php", "Sila gunakan nama lain, nama pentadbir tersebut sudah terdapat di dalam sistem.");
  }
}
?>
              ...
              <div class="container-xxl justify-content-center align-items-center text-center">
                <div class="row">
                  <div class="col-12 p-4">
                    <i class="fa-5x fa-solid fa-user-gear"></i>
                  </div>
                </div>
              </div>
              <h3 class="mb-4 pb-2 pb-md-0 mb-md-5 text-center">DAFTAR MASUK PENTADBIR</h3>

              <ul class="nav nav-pills nav-justified mb-3" id="ex1" role="tablist">
                <li class="nav-item" role="presentation">
                  <a href="<?php echo (PENTADBIR_URL . "/register.php"); ?>" class="nav-link active" id="tab-login" data-mdb-toggle="pill" href="#pills-login" role="tab" aria-controls="pills-login" aria-selected="true">Daftar Masuk</a>
                </li>
                <li class="nav-item" role="presentation">
                  <a href="<?php echo (PENTADBIR_URL . "/login.php"); ?>" class="nav-link" id="tab-register" data-mdb-toggle="pill" href="#pills-register" role="tab" aria-controls="pills-register" aria-selected="false">Log Masuk</a>
                </li>
              </ul>
              ...
              <form autocomplete="off" action="register.php" method="post" enctype="multipart/form-data">
                <div class="form-outline mb-4">
                  <div class="form-floating">
                    <input type="text" name="namapentadbir" class="form-control" placeholder="Nama" />
