...
<?php
    ...
    if($_SESSION["Auth"]){
        if(isset($_POST["submit_tukar_kata_laluan"])){
            ...
            $kataLaluanLamaDB = $loginModel->getAllWardenWhere("id", $_SESSION["Auth"]["id"])[0]["katalaluanwarden"];
            ...
            // Update login model
            if($validInput){
                $loginModel->updateUser("loginwarden", "id", $_SESSION["Auth"]["id"], [
                    "katalaluanwarden" => password_hash($kataLaluanBaharu, PASSWORD_BCRYPT),
                ]);
